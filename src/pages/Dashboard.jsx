...
import { getDashboardSummary, getDashboardOverview } from "../apis/dashboard";
...
export default function Dashboard() {
  ...
  const fetchDashboardData = async (isRefresh = false) => {
    ...
    try {
      setRefreshing(true);
      setError("");
      const [summaryRes, fullRes] = await Promise.allSettled([
        getDashboardSummary(),
        getDashboardOverview(),
      ]);
      if (fullRes.status === "fulfilled") setOverview(fullRes.value);
      else if (summaryRes.status === "fulfilled") setOverview(summaryRes.value);
    } catch (e) {
  ...
  const ordersTrend = useMemo(
    () => salesLast7Days.map((d) => ({ date: fmtDate(d.date), orders: d.orders })),
    [salesLast7Days]
  );
  const revenueTrend = useMemo(
    () => salesLast7Days.map((d) => ({ date: fmtDate(d.date), revenue: d.revenue })),
    [salesLast7Days]
  );
    ...
    () => [
      {
        ...
        icon: FaRupeeSign,
        description: `This month: ${fmtCurrency(summary.monthRevenue)} • Avg order: ${fmtCurrency(summary.avgOrderValue)}`,
        link: null,
      },
      {
        title: "Orders",
      ...
      },
      {
        ...
        icon: FaBoxOpen,
        description: `Active: ${fmtNum(summary.activeProducts)} • Categories: ${fmtNum(summary.totalCategories)}`,
        link: "/products",
      },
      {
        title: "Team",
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="p-6 rounded-xl border" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
                  <h2 className="text-lg font-semibold mb-4 flex items-center justify-between" style={{ color: themeColors.text }}>
                    <span className="flex items-center gap-2"><FaChartBar /> Orders (Last 7 Days)</span>
                    <span className="text-xs opacity-70">Today: {fmtNum(summary.todayOrders)} • Total: {fmtNum(summary.totalOrders)}</span>
                  </h2>
                ...
                <div className="p-6 rounded-xl border" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
                  <h2 className="text-lg font-semibold mb-4 flex items-center justify-between" style={{ color: themeColors.text }}>
                    <span className="flex items-center gap-2"><FaChartBar /> Revenue (Last 7 Days)</span>
                    <span className="text-xs opacity-70">Total: {fmtCurrency(summary.totalRevenue)}</span>
                  </h2>
                  <div style={{ width: "100%", height: 280 }}>
                    <ResponsiveContainer>
              ...
              <div className="p-6 rounded-xl border" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
                <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: themeColors.text }}>
                  <FaChartBar /> Products by Category
                </h2>
                <div style={{ width: "100%", height: 280 }}>
                  <ResponsiveContainer>
              ...
              {/* Tables: Latest Orders & Products */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Latest Orders */}
                <div className="p-6 rounded-xl border" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
                  <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: themeColors.text }}>
                    <FaShoppingCart /> Latest Orders
                  </h2>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr style={{ backgroundColor: themeColors.background + "30" }}>
                          {["Order", "Customer", "Items", "Total", "Status", "Payment", "Created"].map((h) => (
                            <th key={h} className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: themeColors.text }}>
                              {h}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody className="divide-y" style={{ borderColor: themeColors.border }}>
                        {latestOrders.map((o) => {
                          ...
                          const itemText = firstItem
                            ? `${firstItem.productName || firstItem.product?.name || "Item"} x${firstItem.quantity}`
                            : "-";
                          return (
                            <tr key={o._id}>
                              ...
                              <td className="px-4 py-2" style={{ color: themeColors.text }}>
                                ...
                                <div className="text-xs opacity-70">{o.shippingAddress?.city}, {o.shippingAddress?.state}</div>
                              </td>
                              <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>
                                {itemText}
                                {extraItems > 0 && <span className="opacity-60"> +{extraItems} more</span>}
                              </td>
                              <td className="px-4 py-2" style={{ color: themeColors.text }}>{fmtCurrency(o.total)}</td>
                              <td className="px-4 py-2 text-xs">
                                <span className="inline-flex items-center px-2 py-1 rounded-full text-[11px] font-semibold" style={{ backgroundColor: themeColors.primary + "10", color: themeColors.primary }}>
                                  {o.status}
                                </span>
                              </td>
                              <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>
                                <div>{o.paymentMethod}</div>
                                <div className="opacity-70">{o.paymentStatus}</div>
                              </td>
                              <td className="px-4 py-2 text-xs opacity-70" style={{ color: themeColors.text }}>{fmtDate(o.createdAt)}</td>
                            </tr>
                          );
                        })}
                        {latestOrders.length === 0 && (
                          <tr><td colSpan={7} className="px-4 py-4 text-center text-sm" style={{ color: themeColors.text }}>No recent orders.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                ...
                <div className="p-6 rounded-xl border" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
                  <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: themeColors.text }}>
                    <FaBoxOpen /> Latest Products
                  </h2>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr style={{ backgroundColor: themeColors.background + "30" }}>
                          {["Product", "Category", "Price", "Final Price", "Status", "Created"].map((h) => (
                            <th key={h} className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: themeColors.text }}>
                              {h}
                            </th>
                          ))}
                        ...
                        {latestProducts.map((p) => (
                          <tr key={p._id}>
                            <td className="px-4 py-2" style={{ color: themeColors.text }}>
                              ...
                              {p.slug && <div className="text-[11px] opacity-70 font-mono">{p.slug}</div>}
                            </td>
                            <td className="px-4 py-2 text-sm" style={{ color: themeColors.text }}>{p.category?.name || "-"}</td>
                            <td className="px-4 py-2" style={{ color: themeColors.text }}>{fmtCurrency(p.price)}</td>
                            <td className="px-4 py-2" style={{ color: themeColors.text }}>{fmtCurrency(p.finalPrice)}</td>
                            <td className="px-4 py-2">
                              <span
                                className="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold"
                                style={{
                                  backgroundColor: p.isActive ? (themeColors.success || themeColors.primary) + "15" : themeColors.border,
                                  color: p.isActive ? themeColors.success || themeColors.primary : themeColors.text,
                                }}
                              >
                                {p.isActive ? "Active" : "Inactive"}
                              </span>
                            </td>
                            <td className="px-4 py-2 text-xs opacity-70" style={{ color: themeColors.text }}>{fmtDate(p.createdAt)}</td>
                          </tr>
                        ))}
                        {latestProducts.length === 0 && (
                          <tr><td colSpan={6} className="px-4 py-4 text-center text-sm" style={{ color: themeColors.text }}>No recent products.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="p-6 rounded-xl border" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
                  <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: themeColors.text }}>
                    <FaEnvelopeOpenText /> Recent Enquiries
                  </h2>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr style={{ backgroundColor: themeColors.background + "30" }}>
                          {["Name", "Email", "Phone", "Subject", "Status", "Created"].map((h) => (
                            <th key={h} className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: themeColors.text }}>
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody className="divide-y" style={{ borderColor: themeColors.border }}>
                        {recentEnquiries.map((e) => (
                          <tr key={e._id}>
                            <td className="px-4 py-2" style={{ color: themeColors.text }}>{e.name}</td>
                            <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>{e.email}</td>
                            <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>{e.phone}</td>
                            <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>{e.subject}</td>
                            <td className="px-4 py-2 text-xs">
                              <span
                                className="inline-flex items-center px-2 py-1 rounded-full text-[11px] font-semibold"
                                style={{
                                  backgroundColor: e.isRead ? themeColors.border : themeColors.primary + "10",
                                  color: e.isRead ? themeColors.text : themeColors.primary,
                                }}
                              >
                                {e.status}
                              </span>
                            </td>
                            <td className="px-4 py-2 text-xs opacity-70" style={{ color: themeColors.text }}>{fmtDate(e.createdAt)}</td>
                          </tr>
                        ))}
                        {recentEnquiries.length === 0 && (
                          <tr><td colSpan={6} className="px-4 py-4 text-center text-sm" style={{ color: themeColors.text }}>No enquiries yet.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                ...
                <div className="p-6 rounded-xl border" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr style={{ backgroundColor: themeColors.background + "30" }}>
                          {["Code", "Title", "Type", "Value", "Min Order", "Max Discount"].map((h) => (
                            <th key={h} className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: themeColors.text }}>
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody className="divide-y" style={{ borderColor: themeColors.border }}>
                        {activeOffers.map((o) => (
                          <tr key={o._id}>
                            <td className="px-4 py-2 font-mono text-xs" style={{ color: themeColors.text }}>{o.code}</td>
                            <td className="px-4 py-2" style={{ color: themeColors.text }}>{o.title}</td>
                            <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>{o.discountType === "percentage" ? "Percentage" : "Flat"}</td>
                            <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>{o.discountType === "percentage" ? `${o.discountValue}%` : fmtCurrency(o.discountValue)}</td>
                            <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>{o.minOrderAmount ? fmtCurrency(o.minOrderAmount) : "-"}</td>
                            <td className="px-4 py-2 text-xs" style={{ color: themeColors.text }}>{o.maxDiscountAmount ? fmtCurrency(o.maxDiscountAmount) : "-"}</td>
                          </tr>
                        ))}
                        {activeOffers.length === 0 && (
                          <tr><td colSpan={6} className="px-4 py-4 text-center text-sm" style={{ color: themeColors.text }}>No active offers.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
