  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanedForm = {
        ...
        type: form.type,
        status: form.status,
        purifiers: form.purifiers.filter(p => p.brand || p.model),
        amcDetails: form.amcDetails.planName ? {
          ...form.amcDetails,
          amountPaid: form.amcDetails.amountPaid ? Number(form.amcDetails.amountPaid) : 0,
          startDate: form.amcDetails.startDate || undefined,
          endDate: form.amcDetails.endDate || undefined,
        } : undefined,
        rentalDetails: form.rentalDetails.planName ? {
          ...form.rentalDetails,
          amount: Number(form.rentalDetails.amount) || 0,
          startDate: form.rentalDetails.startDate || undefined,
          nextDueDate: form.rentalDetails.nextDueDate || undefined,
        } : undefined,
      };

      if (editing) {
        await updateCustomer(editing._id, cleanedForm);
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex flex-col gap-1 items-start">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${cust.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                            }`}>
                            {cust.status}
                          </span>
                      ...
                      <td className="p-4">
                        <span className="text-[10px] font-bold opacity-60 px-2 py-1 bg-blue-50 text-blue-700 rounded-full border border-blue-100 flex items-center justify-center w-fit">
                          {cust.source || "N/A"}
                        </span>
                      </td>
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() => navigate(`/customer-detail/${cust._id}`)}
            ...
            <div className="flex border-b" style={{ borderColor: themeColors.border }}>
              {['basic', 'purifier', 'amc', 'rental'].map(tab => (
                <button
                  ...
                  className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab ? 'border-blue-500 text-blue-500' : 'border-transparent opacity-60 hover:opacity-100'
                    }`}
                >
