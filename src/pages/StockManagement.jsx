...
export default function StockManagement() {
    ...
    return (
        ...
        >
                        ...
                        <form onSubmit={handleUpdateSubmit} className="space-y-4">
                            <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-md font-medium text-sm flex items-center justify-center gap-2 transition ${updateForm.type === 'Add'
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-md font-medium text-sm flex items-center justify-center gap-2 transition ${updateForm.type === 'Remove'
            ...
            {isAddStockModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="rounded-xl shadow-2xl max-w-4xl w-full p-6 max-h-[85vh] overflow-hidden flex flex-col"
                        style={{ backgroundColor: themeColors.surface, color: themeColors.text }}
                    >
                        <div className="flex justify-between items-center mb-4 border-b pb-4" style={{ borderColor: themeColors.border }}>
                            <h2 className="text-xl font-bold flex items-center gap-2">
                                <FaPlus className="text-blue-500" /> Add Stock to Products
                            </h2>
                            <button onClick={() => {
                                setIsAddStockModalOpen(false);
                                setStockUpdates({});
                            }} className="text-2xl opacity-50 hover:opacity-100">&times;</button>
                        </div>

                        <div className="overflow-y-auto flex-1 pr-2">
                            {products.length === 0 ? (
                                <div className="text-center py-10 opacity-50">No products available.</div>
                            ) : (
                                        ...
                                        <div key={product._id} className="p-4 rounded-lg border flex items-center justify-between gap-4" style={{ borderColor: themeColors.border, backgroundColor: themeColors.background }}>
                                            <div className="flex items-center gap-3 flex-1">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-xs opacity-50">Current Stock: {product.stock} units</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    className="w-24 p-2 rounded-lg border outline-none focus:ring-2 focus:ring-blue-500 text-center font-bold"
                                                    style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border, color: themeColors.text }}
                                                />
                                            </div>
                                        </div>
                        ...
                        <div className="flex justify-end gap-3 mt-6 pt-4 border-t" style={{ borderColor: themeColors.border }}>
                            ...
                            <button
                                type="button"
                                onClick={async () => {
                                    const updates = Object.entries(stockUpdates).filter(([_, qty]) => qty && Number(qty) > 0);
                                    if (updates.length === 0) {
                                        Swal.fire("Info", "Please enter quantities to update", "info");
                                        return;
                                    }
                                    ...
                                    try {
                                        for (const [productId, quantity] of updates) {
                                            await updateStock({
                                                ...
                                                type: "Add",
                                                reason: "New Shipment",
                                                note: "Bulk stock addition"
                                            });
                                        }
                                        Swal.fire("Success", `Stock updated for ${updates.length} product(s)`, "success");
                                        setIsAddStockModalOpen(false);
                                        setStockUpdates({});
                                        fetchData();
                                    } catch (err) {
